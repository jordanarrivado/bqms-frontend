...
const AddArea = ({ 
  ...
  const handleEditArea = async (area) => {
    ...
    if (newName && newName !== area.name) {
      try {
        const response = await axios.put(
          `https://bqms-backend.onrender.com/users/${user.email}/areas/${area._id}`, 
          { name: newName }
        );
  ...
  const handleDeleteArea = async (areaId) => {
    setLoading(true);
    try {
      const response = await axios.delete(`https://bqms-backend.onrender.com/users/${user.email}/areas/${areaId}`);
