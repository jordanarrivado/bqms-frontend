...
const CourtCon = ({ user, courts, setCourts, inMatch, selectedBall, isChecked, courtLoading, handleWinner,setCourtLoading }) => {
  ...
  const handleAddCourt = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `https://bqms-backend.onrender.com/users/${user.email}/areas/${localArea}/courts`
      );
  ...
  const confirmDeleteCourt = async (index) => {
    ...
    try {
      ...
      setCourtLoading(true);
      await axios.delete(
        `https://bqms-backend.onrender.com/users/${user.email}/areas/${localArea}/courts/${courtToDelete._id}`
      );
