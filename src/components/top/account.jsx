...
const Account = ({ logout, user, setSessionStart, sessionStart, textCode, setTextCode, qrCode, setQrCode }) => {
  ...
  useEffect(() => {
    const fetchQRCode = async () => {
      ...
      try {
        ...
        const response = await axios.get(`https://bqms-backend.onrender.com/users/${user.email}/areas/${localArea}/sessions/${encodeURIComponent(sessionDate)}`);
  ...
  const handleProfileUpdate = async () => {
    try {
      ...
      const response = await axios.put(`https://bqms-backend.onrender.com/profile/${user._id}`, formData, {
        headers: {
